...
Publication-ready Aquila figures are generated by
[`aquila/analyze_aquila_results.py`](aquila/analyze_aquila_results.py) and are
available as PNG, PDF, and SVG files in [`aquila/figures/`](aquila/figures/).
Error bars are 95% confidence intervals calculated from the shots retained after
successful atom loading: Wilson intervals for `P(n=1)`, normal-approximation
intervals for means, and bootstrap intervals for `g2(1)`. The summary records `m_valid` (retained shots),
`m_submitted` (submitted shots), and `loading_rate`; exact values are retained in
[`aquila/data/aquila_plot_summary.json`](aquila/data/aquila_plot_summary.json).
The plotted `P(n=1)` is the probability of exactly one excitation, and the
`1/N` curve is a reference, not a fit.
